...
USER = ""  # Will store username
FIRSTNAME = ""  # Will store firstname
UserGender = ""
userWin = 0
compWin = 0
counter = 0
...
def play():
    ...
    Scoreboard = Label(root, text="SCORE \n\n    " + FIRSTNAME.upper() + " - " + str(userWin) + "\t\tCOMPUTER - " + str(compWin), bg='blue',
...
# Function containing the whole logic of won-lose-tie in the game
def youPick(yourChoice):
    ...
    if click:
        ...
        if yourChoice in ['rock', 'paper', 'scissors']:
    ...
    if counter == turns:
        ...
        confetiImg = PhotoImage(file=img_paths["confeti"])
...
# Reset the frame to original starting pictures
def reset_frame():
    global click
    rHandButton.configure(image=rHandPhoto)
    pHandButton.configure(image=pHandPhoto)
...
def start():
    ...
    # Load images using img_paths
    rHandPhoto = PhotoImage(file=img_paths["rHand"])
    pHandPhoto = PhotoImage(file=img_paths["pHand"])
    sHandPhoto = PhotoImage(file=img_paths["sHand"])
    rock = Image.open(img_paths["rock"])
    rockImage = ImageTk.PhotoImage(rock)
    paper = Image.open(img_paths["paper"])
    paperImage = ImageTk.PhotoImage(paper)
    scissors = Image.open(img_paths["scissors"])
    scissorImage = ImageTk.PhotoImage(scissors)
    win = Image.open(img_paths["win"])
    winImage = ImageTk.PhotoImage(win)
    lose = Image.open(img_paths["lose"])
    loseImage = ImageTk.PhotoImage(lose)
    tie = Image.open(img_paths["tie"])
...
def welcomeUserPage():
    ...
    welcomeUser.config(font=("Times", 20, "bold"))  # Fixed font typo
    welcomeUser.grid(row=0, column=0, columnspan=2)

    if UserGender == "Male":
        maleimg = ImageTk.PhotoImage(Image.open(img_paths["male"]))
        maleLabel = Label(root, image=maleimg)
        maleLabel.image = maleimg  # Keep reference to avoid garbage collection
        maleLabel.grid(row=1, column=0)
    else:
        femaleimg = ImageTk.PhotoImage(Image.open(img_paths["female"]))
        femaleLabel = Label(root, image=femaleimg)
        femaleLabel.image = femaleimg
        femaleLabel.grid(row=1, column=0)

    compimg = ImageTk.PhotoImage(Image.open(img_paths["computer"]))
...
def check():
    global username, password, USER, FIRSTNAME, UserGender
    conn = sqlite3.connect('Users.db')
    c = conn.cursor()
    c.execute("SELECT * FROM Users WHERE username = :user AND password = :pass",
              {'user': username.get(), 'pass': password.get()})
    data = c.fetchone()
    conn.commit()
    conn.close()
    ...
    if username.get() == "" or password.get() == "" or data is None:
        ...
        FIRSTNAME = str(data[0])  # Set to firstname (index 0)
        USER = str(data[2])       # Set to username (index 2)
        UserGender = data[4]      # Gender
        print(f"Logged in - Username: {USER}, Firstname: {FIRSTNAME}")  # Debug print
        welcomeUserPage()
    return

# # Function to create USERS TABLE (run only once)
def create():
    conn = sqlite3.connect('Users.db')
    c = conn.cursor()
    c.execute("""
...
# # Function to create LeaderBoard Table (run only once)
...
# Uncomment and run these once, then comment them out again
# create()
# LeaderboardCreate()

# Function to INSERT data into the Leaderboard table
def insertToLeaderBoard():
    global FIRSTNAME, userWin, compWin
    try:
        conn = sqlite3.connect('LeaderBoard.db')
        c = conn.cursor()
        print(f"Firstname: {FIRSTNAME}, userWin: {userWin}, compWin: {compWin}")  # Debug print
        
        c.execute("SELECT * FROM LEADERBOARD WHERE firstname = :firstname", {'firstname': FIRSTNAME})
        data = c.fetchone()
        print(f"Existing data: {data}")  # Debug print
        
        if data:
            won, lost, tied, played = data[1], data[2], data[3], data[4]
            if userWin > compWin:
                won += 1
            elif userWin < compWin:
                lost += 1
            ...
            print(f"Updating: {won}, {lost}, {tied}, {played}, {rate}")  # Debug print

            c.execute("""
                UPDATE LEADERBOARD SET
                GamesWon = ?, GamesLost = ?, GamesTied = ?, GamesPlayed = ?, WiningRate = ?
                WHERE firstname = ?""", (won, lost, tied, played, rate, FIRSTNAME))
        else:
            won = 1 if userWin > compWin else 0
            lost = 1 if userWin < compWin else 0
            tied = 1 if userWin == compWin else 0
            played = 1
            rate = round(won / played, 2) if played > 0 else 0
            print(f"Inserting new: {won}, {lost}, {tied}, {played}, {rate}")  # Debug print
            
            c.execute("""
                INSERT INTO LEADERBOARD (firstname, GamesWon, GamesLost, GamesTied, GamesPlayed, WiningRate)
                VALUES (?, ?, ?, ?, ?, ?)""", (FIRSTNAME, won, lost, tied, played, rate))
        
        conn.commit()
        
        # Verify the update worked
        c.execute("SELECT * FROM LEADERBOARD WHERE firstname = :firstname", {'firstname': FIRSTNAME})
        print(f"After update: {c.fetchone()}")  # Debug print
...
# Function to insert data into the USERS database
def insert():
    ...
    c.execute("SELECT * FROM USERS WHERE username = ?", (username.get(),))
    if c.fetchone():
    ...
    # Insert new user
    c.execute(
        "INSERT INTO USERS (firstname, lastname, username, password, gender) VALUES (?, ?, ?, ?, ?)",
    ...
    c2 = conn2.cursor()
    c2.execute(
        "INSERT INTO LEADERBOARD (firstname, GamesWon, GamesLost, GamesTied, GamesPlayed, WiningRate) VALUES (?, 0, 0, 0, 0, 0)",
        (fname.get(),)
    )
    conn2.commit()
    conn2.close()
    ...
    login()  # Switch to login page after signup
    return
